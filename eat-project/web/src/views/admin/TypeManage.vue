<template>
  <div>
    ...
    <el-table ref="multipleTable"
      ...
      <el-table-column prop="timageurl" label="类型图片">
        <template slot-scope="scope">
          <el-image
              style="width: 60px;height: 60px"
              :src="scope.row.timageurl"
              :preview-src-list="[scope.row.timageurl]"
          >
          </el-image>
        </template>
      </el-table-column>
        ...
        <el-form :model="formData" label-width="100px" class="demo-ruleForm">
          <slot name="content">
            ...
            <el-form-item label="类型图片">
              ...
              >
                <img v-if="formData.timageurl" :src="formData.timageurl" alt="" style="width: 150px;height: 150px">
...
export default {
  ...
  methods: {
    ...
    submitDrawer() {
      //新增
      if (!this.formData.id) {
        this.request.post("/type", this.formData).then(res => {
          console.log(res)
          if (res.code === 200) {
            this.isDrawerDialog = false
            this.$message({
        ...
        //更新
        this.request.put("/type", this.formData).then(res => {
          console.log(res)
          if (res.code === 200) {
            this.isDrawerDialog = false
            this.$message({
    ...
    batchDelete(rows) {
      ...
      this.$confirm(`此操作将永久删除ID为[${ids}]的数据, 是否继续?`, '提示', {
        ...
        //批量删除数据
        this.request.delete("/type/", {data: ids}).then(res => {
          if (res.code === 200) {
            this.$message({
              message: res.msg,
              type: 'success'
    ...
    uploadImgSuccess(res) {
      this.$set(this.formData, "timageurl", res.data.url)
    },
  }
};
</script>
